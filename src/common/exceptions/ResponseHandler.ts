import { Response } from "express";

export function ResponseHandler(
  res: Response,
  status: number,
  message: string,
  data: any = null
) {
  res.status(status).json({
    success: status < 400,
    status,
    message,
    data,
  });
}

export interface IApiError {
  [key: string]: string;
}

export const AppError: IApiError = {
  EMAIL_IN_USE: "Email already in use",
  INVALID_CREDENTIALS: "Invalid credentials",
  FILE_ERROR: "No files uploaded",
  TOKEN_EXPIRED: "Token is invalid or has expired",
  FIELD_ERROR: "All fields are required",
  NOT_FOUND: "Resource not found",
  SELLER_AUTH: "Only a seller can perform this action",
  UNAUTHORIZED: "You are not authorized to access this resource",
  CART_NOT_FOUND: "Cart not found",
  PRODUCT_NOT_FOUND: "Product not found",
  PRODUCT_NOT_IN_CART: "Product is not in the cart",
  INVALID_QUANTITY: "Invalid quantity requested",
  INSUFFICIENT_STOCK: "Not enough stock available",
  INVALID_PRODUCT_ID: "Invalid product ID",
  SERVER_ERROR: "Internal server error",
  INVALID_STATUS: "Invalid Product Status",
  EMAIL_PASS_ERROR: "Email and password are required",
  USER_EXIST: "User already exists",
  USER_NOT_FOUND: "User not found",
  INVALID_ROLE: "Invalid role",
  EMPTY_CART: "Your cart is empty",
  MAP_LOCATION: "Mapping location is required",
  INVALID_MAP_LOCATION: "Invalid mapping location",
  TRANSACTION_HASH_REQUIRED: "Transaction Hash is require",
  PURCHASE_FAILED: "Failed to complete payment",
  INVALID_FILE: "Invalid document file or image",
  PRODUCT_DEL: "Cannot delete product with remaining stock",
  XION_BALANCE: "Error fetching balance",
  ERROR_XION: "Error",
  XION_CONTRACT: "Error Cannot deposit. Contract status is",
  INVALID_CONTRACT_STATE:"Invalid contract state",
  XION_DEPOSIT_ERROR:'xion deposit error',
  XION_EXECUTE_ERROR:'Xion execute error',
  UPLOAD_FAILED:'Upload failed',
  DEPLOY_FAILED:'Deploy failed'
} as const;

export type ErrorKey = keyof typeof AppError;
export type ErrorTyepOf = typeof AppError;
export type ErrorKeyOf = keyof IApiError;

export function ErrorHandler(
  res: Response,
  errorKey: keyof IApiError,
  statusCode: number
) {
  const errorMessage = AppError[errorKey] || "An unexpected error occurred";

  console.error(
    `❌ ErrorHandler Triggered: ${errorKey} (${statusCode}) -> ${errorMessage}`
  );

  return ResponseHandler(res, statusCode, errorMessage);
}

//? instanceof
//? typeof
//? in
//? is
//?  as
//? keyof
// export interface IApiError {
//     [key: string]: string;
//   }
// export type IApiError = Record<string, string>;
// utils/errorMap.ts
// const errorMap: Record<string, { status: number; message: string }> = {
//   "Unauthorized": { status: 401, message: "Unauthorized" },
//   "User not found": { status: 404, message: "User not found" },
//   // ... other mappings
// };

// export const getHttpError = (error: Error) => {
//   return errorMap[error.message] || { status: 500, message: "Internal server error" };
// };

// // In your route handler:
// const { status, message } = getHttpError(error);
// return ErrorHandler(res, message, status);
