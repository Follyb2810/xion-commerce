...
import { KYCStatus, Roles } from "./../../common/types/IUser";
...
export const uploadKycDocuments = AsyncHandler(
  async (req: AuthRequest, res: Response) => {
    ...
    if (!uploadKyc?.secure_url)
      return ErrorHandler(res, "Cloud upload failed", 500);
...
export const getKycStatus = AsyncHandler(
  async (req: AuthRequest, res: Response): Promise<void> => {
    ...
    const user = await UserService.findById(userId, "kyc");
    if (!user) return ErrorHandler(res, "USER_NOTFOUND", 404);
